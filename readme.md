...
- Flask app auto-starts via the `artifacts/accessease: web` workflow
- `cd artifacts/accessease && python run.py` — run Flask manually (port from PORT env)
- Admin login: username=`admin` password=`Admin@123`
- DB is auto-initialized (SQLite) on first run — file: `artifacts/accessease/accessease.db`
